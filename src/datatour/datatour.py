...
class DataTour:
    def __init__(self, data=None,
        ...
        Args:
            ...
            labels(array): 1d array [sample_idx] of class idx - used only for display color coding with color='l' flag
        ...
        self.features, self.features_v, self.labels = self.prepare_features(data=data, data_v=data_v,
    ...
    def prepare_features(data, data_v, labels, norm, n_subsample):
        ...
        features, features_v, labels = subsample_features(f=features, f_v=features_v,
                                                          labels=labels, n_samples=n_subsample)
    ...
    def display(self,
        ...
        """
        Args:
            color(str): None or one of self.dict keys, e.g. 'l' - labels or 'z_scaled',
    ...
    def display_quiver(self,
        ...
        for i, t_id in enumerate(t_ids):
            ...
            quiver_data = ff.create_quiver(xt, yt, ut, vt,
                                           scale=uv_scale,
                                           arrow_scale=arrow_scale,
                                           angle=angle,
                                           line=dict(width=0.75, color='lightsteelblue'),
                                           opacity=opacity
                                           ).data[0]
            ...
            if point_size == 0:
                ...
                scatter_data = go.Scatter(x=xt, y=yt,
                                          name='Location',
                                          mode='markers',
                                          marker=dict(color=ct,
                                                      ...
                                                      colorbar=dict(thickness=10, tickvals=[0, 1],
                                                                    ticktext=['Far', 'Close'], outlinewidth=0)
                                                      )
                                          )

                # Adding frames
            data = [quiver_data] if scatter_data is None else [quiver_data, scatter_data]
            traces = [0] if scatter_data is None else [0, 1]

            frames.append(dict(name=i, data=data, traces=traces))
        ...
        layout = go.Layout(width=fig_size,
                           ...
                           updatemenus=[dict(type='buttons', showactive=False,
                                             ...
                                             buttons=[dict(label='Play',
                                                           method='animate',
                                                           args=[None,
                                                                 dict(frame=dict(duration=5, redraw=False),
